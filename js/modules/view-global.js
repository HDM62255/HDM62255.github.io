...
function renderGlobalListAsync(filterLabel) {
    ...
    function renderChunk() {
        ...
        for (let i = currentIndex; i < limit; i++) {
            ...
            tr.innerHTML = `<td><div class="fw-bold text-start">${u.name}</div></td><td><div class="small text-muted">${u.id}</div></td><td>${dateTerm}</td><td>${avgProd}</td><td><span class="eff-val ${effStyle.textClass}">${avgEff}%</span></td><td><span class="badge-gxo ${style.badgeClass}">${label}</span></td>`;
...
function exportGlobalListToXLSX() {
    ...
    const data = globalUserListCache.map(u => {
        const avgProd = u.countProd > 0 ? (u.sumProd / u.countProd).toFixed(1) : 0;
        const avgEff = u.countEff > 0 ? (u.sumEff / u.countEff).toFixed(1) : 0;

        // Recalculate Category Logic (same as render)
        const sBest = u.cats.BEST * 0.3;
        const sNorm = u.cats.NORMAL * 0.3;
        const sWorst = u.cats.WORST * 0.4;
        let label = "NORMAL";
        if (sWorst >= sBest && sWorst >= sNorm) label = "WORST";
        else if (sBest >= sNorm && sBest > sWorst) label = "BEST";

        return {
            "ID": u.id,
            "Usuario": u.name,
            "Prod. Real": parseFloat(avgProd),
            "Eficiencia %": parseFloat(avgEff),
            "Categoría": label
        };
    });
