...
export function processBulk() {
    ...
    tokens.forEach(token => {
        ...
        results.push({ id: uRows[0].id, name: uRows[0].name, main: Object.keys(counts).reduce((a, b) => counts[a] > counts[b] ? a : b), eff: cntEff > 0 ? (sumEff / cntEff) : 0, turno: uRows[0].turno || "-" });
    });
    ...
    results.forEach(r => {
        ...
        tr.innerHTML = `<td><div class="fw-bold">${r.name}</div><div class="small text-muted">${r.id}</div></td><td>${r.turno}</td><td class="fw-bold">${r.main}</td><td class="text-center"><span class="eff-val ${style.textClass}">${r.eff.toFixed(1)}%</span></td><td class="text-center"><span class="badge-gxo ${style.badgeClass}">${style.label}</span></td>`;
